...
class EventSchemaInjector {
    ...
    private function load_csv_data() {
        ...
        $this->csv_data_cache = $event_data;
        return $event_data;
    }

    private function find_events_for_url($csv_data, $canonical_url) {
        $matching_events = array();
        foreach ($csv_data as $event_row) {
            if (isset($event_row['MainPageURL']) && trim($event_row['MainPageURL']) === $canonical_url) {
                $matching_events[] = $event_row;
            }
        }
